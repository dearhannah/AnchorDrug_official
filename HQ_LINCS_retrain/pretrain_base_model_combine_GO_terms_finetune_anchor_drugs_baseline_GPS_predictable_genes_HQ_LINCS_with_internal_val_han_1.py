...
def main(args):
    ...
    for rdseed in [10]: #set seed = 10
        ...
        if args.input == 'drugcellline':
            finetune_dataset = DrugCellline(
                df=df_finetune, type = 'train',
                ...
                down_sample=True)
            test_dataset = DrugCellline(
                df=df_ext_test_GO_cellline, type = 'test',
                fn_file = '/egr/research-aidd/menghan1/AnchorDrug/data/CellLineEncode/test_cell_line_expression_features_128_encoded_20240111.csv', 
                down_sample=False)
            val_dataset = DrugCellline(
                df= df_test_GO_cellline, type = 'test', 
                fn_file = '/egr/research-aidd/menghan1/AnchorDrug/data/CellLineEncode/test_cell_line_expression_features_128_encoded_20240111.csv', 
                down_sample=False)
        ...
        train_loader = torch.utils.data.DataLoader(
            dataset=finetune_dataset, batch_size=32, num_workers=0,
            drop_last=False, shuffle=True)
        test_loader = torch.utils.data.DataLoader(
            dataset=test_dataset, batch_size=test_dataset.data.shape[0], num_workers=0,
            drop_last=False, shuffle=False)
        val_loader = torch.utils.data.DataLoader(
            dataset=val_dataset, batch_size=val_dataset.data.shape[0], num_workers=0,
            drop_last=False, shuffle=False)
...
gene = pd.read_csv('/egr/research-aidd/menghan1/AnchorDrug/HQ_LINCS_retrain/GPS_predictable_genes.csv')['x'].tolist()#[:4]

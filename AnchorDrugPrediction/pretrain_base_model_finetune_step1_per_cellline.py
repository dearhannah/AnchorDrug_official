...
for g in gene:
    ...
    #----------------------------------------------------------------------------------------------------------------
    #Import finetune data (data per cell line in the pre-trained data):
    c = cell_list #51 pre-trained cell lines
    for cell in c: #Each cell line fine-tune one model:
        df_finetune = df_train.loc[df_train['cellline'] == cell, :]
        #
        #----------------------------------------------------------------------------------------------------------------
    ...
    #----------------------------------------------------------------------------------------------------------------
        def main(args):
            ...
            for rdseed in [10]: #set seed = 10
                ...
                #net = torch.load('/egr/research-aidd/chenruo4/AnchorDrug/base_model/revise_MLP_128_64_32_normalized_cell_line_embeddings_ECFP/pretrain_universal_gene_' + g + '_seed_10_199_final.pth')
                ...
                #
                # out setting
                model_str = 'pretrain_universal_'+'gene_' + g + '_cellline_' + cell + '_seed_' + str(rdseed)
                #freeze some layers:
                ...
                net.load_state_dict(torch.load('/egr/research-aidd/chenruo4/AnchorDrug/base_model/revise_MLP_128_64_32_normalized_cell_line_embeddings_ECFP/pretrain_universal_gene_' + g + '_seed_10_199_final.pth').state_dict())
                #optimizer = torch.optim.Adam(net.parameters(), lr=args.lr)
                optimizer = torch.optim.Adam(filter(lambda m: m.requires_grad, net.parameters()), lr=args.lr)
                #
                summary_train_loss = []
                ...
                pd.DataFrame({'training_loss': summary_train_loss}, index = range(0, e+1)).to_csv(out_dir + g + '_' + cell + '_finetune_loss_per_epoch.csv')
                torch.save(net, out_dir + model_str + '_%s_final.pth' % str(e))
    #----------------------------------------------------------------------------------------------------------------
        if __name__ == '__main__':
            argparser = argparse.ArgumentParser()
            argparser.add_argument('--out_dir', type=str, help='dir to output', default='/egr/research-aidd/chenruo4/AnchorDrug/base_model/revise_MLP_128_64_32_normalized_cell_line_embeddings_ECFP_finetune_stpe1_per_pretraining_cellline_20epoch/')
